...
async function changeLayerTitle(layerId: string, newTitle: string) {
  const response = await pool.query(
    `
    UPDATE layers 
    SET title = $2
...
async function deleteRelations(layerId: string, relsIds: string[]) {
...
async function saveLayer(userId: string, title: string, relations: Relation[]) {
  const client = await pool.connect();
...
async function getLayerRelations(layerId: string) {
