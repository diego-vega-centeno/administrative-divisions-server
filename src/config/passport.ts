  ...
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, done) {
      try {
        // find user
        let user;
        user = await pool.query("SELECT * FROM users WHERE oauth_id = $1", [
          "google-" + profile.id,
        ]);
  ...
  new OAuth2Strategy(
    {
      ...
      callbackURL:
        process.env.NODE_ENV === "development"
          ? process.env.CALLBACK_DEV_OSM_URL
          : process.env.CALLBACK_PROD_OSM_URL,
      scope: ["read_prefs"],
    },
    async function (
      accessToken: string,
      ...
      try {
        ...
        const userData = (await res.json()) as OSMUserResponse;
