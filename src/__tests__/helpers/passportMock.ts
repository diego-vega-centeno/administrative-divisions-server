...
let originalGoogleStrategy: Strategy | null = null;
...
const mockGoogleStrategy: Strategy = {
  name: "google",

  authenticate: function (req: Request) {
    const CALLBACK_URL =
      process.env.NODE_ENV === "development"
        ? process.env.CALLBACK_DEV_GOOGLE_URL!
        : process.env.CALLBACK_PROD_GOOGLE_URL!;
    const FRONTEND_URL =
      process.env.NODE_ENV === "development"
        ? process.env.FRONTEND_DEV_URL
        : process.env.FRONTEND_DEV_PROD;
...
export function installMockGoogleStrategy() {
  ...
  originalGoogleStrategy = (passport as any)._strategies["google"] || null;
